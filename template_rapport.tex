...
\begin{titlepage}
    \begin{center}
        ...
        Tuteur école : %Marc Médale, Professeur AMU-Polytech'Marseille
        %\par\end{center}
        %\end{minipage}}
    \end{center}
\end{titlepage}
...
\begin{figure}
	\centering
	\begin{tikzpicture}
	\node[draw,rectangle,diamond, aspect=1.3, text centered,text width=1.5cm] (T) at (0,2) {$t$ = $t^n$ ? };
	...
	\node[text centered,text width=1cm,color=black] (K)at(0.5,-5.9){oui};
	\node[text centered,text width=1cm,color=black] (K)at(-1.5,-4.8){non};
